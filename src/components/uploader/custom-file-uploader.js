...
const emits = [ 'factory-failed', 'uploaded', 'failed', 'uploading', 'state-initialized' ]
...
function injectPlugin ({ props, emit, helpers }) {
  ...
  const emitToParent = () => {
    emit('state-initialized', reactive(helpers))
  }
  onMounted(() => {
    emitToParent()
  })
...
export default createUploaderComponent(CustomUploader)
