  ...
  <div v-if="init">
    <q-card v-if="appStore.isAuthorized">
      ...
      <q-card-actions vertical align="center">
        ...
        <q-btn type="a" :to="{name:'flat-sale'}" flat>{{$t('authPageEstimateSaleLink')}}</q-btn>
        <q-btn type="a" :to="{name:'flat-rent'}" flat>{{$t('authPageEstimateRentLink')}}</q-btn>
        <q-btn type="a" :to="{name:'logout'}" flat>{{$t('authPageLogoutLink')}}</q-btn>
      </q-card-actions>
    </q-card>
    <AuthComplex :loginBlockFirst="true" v-else @register-result="registerResult" @login-result="loginResult"></AuthComplex>
    </div>
</template>
...
export default {
  ...
  setup () {
    ...
    onMounted(() => {
      loadAuthState().finally(() => init.value = true)
    })
